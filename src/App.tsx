...
function SintFrame({ W = 270, H = 330 }) {
  ...
  const KRANS = [
    ...
    { a: 104, e: '⭐', fs: 12, off: 8  },
    { a: 117, e: '🍊', fs: 15, off: 4  },
    { a: 130, e: '🎁', fs: 17, off: 12 },
    { a: 143, e: '🍫', fs: 14, off: 3  },
    { a: 156, e: '⭐', fs: 13, off: 9  },
    { a: 169, e: '🍊', fs: 15, off: 4  },
    { a: 182, e: '⭐', fs: 21, off: 13 },
    { a: 195, e: '🎶', fs: 13, off: 4  },
    { a: 208, e: '🍊', fs: 16, off: 11 },
    { a: 221, e: '🎁', fs: 15, off: 3  },
    { a: 234, e: '⭐', fs: 12, off: 9  },
    { a: 247, e: '🍊', fs: 15, off: 5  },
    { a: 260, e: '🍭', fs: 14, off: 12 },
    { a: 273, e: '🍊', fs: 15, off: 3  },
    { a: 286, e: '⭐', fs: 12, off: 8  },
    { a: 299, e: '🍫', fs: 14, off: 5  },
    { a: 312, e: '🎁', fs: 15, off: 11 },
    { a: 325, e: '🍊', fs: 15, off: 4  },
    { a: 338, e: '⭐', fs: 12, off: 8  },
    { a: 351, e: '🧸', fs: 15, off: 4  },
  ];
  ...
  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}
      ...
      <defs>
        ...
        <filter id="fSh">
          <feDropShadow dx="0" dy="1.5" stdDeviation="2" floodColor="#1a0008" floodOpacity="0.6"/>
        </filter>
      </defs>
      ...
      {KRANS.map((p, i) => {
        ...
        return (
          <text key={`k${i}`} x={px} y={py} fontSize={p.fs}
            textAnchor="middle" dominantBaseline="middle"
            filter="url(#fSh)" style={{ userSelect: 'none' }}>{p.e}</text>
        );
      })}
      ...
      {OVERHANGERS.map((p, i) => {
        const [px, py] = ptE(p.a, rx + p.off, ry + p.off);
        return (
          <text key={`o${i}`} x={px} y={py} fontSize={p.fs}
            textAnchor="middle" dominantBaseline="middle"
            style={{ userSelect: 'none' }} opacity="0.82">{p.e}</text>
        );
      })}
...
// ── Gedeelde stijlen ──────────────────────────────────────────────────────
const bubble = {
  background: 'linear-gradient(160deg,rgba(42,5,8,0.98),rgba(22,3,5,0.99))',
...
export default function SinterklasSpiegel() {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      setIsSpeaking(true);
      speak('Vol verwachting klopt ons hart! Welkom bij de Sinterklaas Spiegel!', () => setIsSpeaking(false));
    }, 1200);
            ...
            <motion.div key="teller" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} style={{ width:'100%' }}>
              <div style={bubble}>
                ...
                <div style={{ textAlign:'center', marginTop:14 }}>
                  <button onClick={()=>setMode(MODE.HOME)} style={{ ...btnR, fontSize:12 }}>← Terug naar Sint</button>
                </div>
              </div>
            </motion.div>
          )}
